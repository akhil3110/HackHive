...
const ChallengeEdit = ( {params} : {params: {challengeId : string}}) => {

    const [initaialData, setInitialData] = useState<initialDataTypes>()
    ...
    useEffect(() => {
        if (initaialData) {
          form.reset({
            challengeName: initaialData.challengeName,
            description: initaialData.description,
            startDate: initaialData.startDate,
            endDate: initaialData.endDate,
            imageUrl: initaialData.imageUrl || "",
            levelType: initaialData.levelType,
          });
        }
    }, [initaialData]);
    ...
    const onSubmit =async (data: z.infer<typeof formSchema>) =>{
        try {
            ...
            const response = await axios.put('/api/challenge', updatedData)
            toast.success("Challenge Updated")
            router.push(`/challenge/${params.challengeId}`)
        } catch (error: any) {
                ...
                <div className="p-5 md:p-0  md:ml-9">
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8" >
                            ...
                            />
                            <div className=" pb-5">
                                <Button className="bg-[#44924C] hover:bg-[#28562c]" type="submit" size="lg">
                                    Save Changes
                                </Button>  
                            </div>
                        </form>
